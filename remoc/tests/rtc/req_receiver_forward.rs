...
use remoc::{
    ...
    rtc::{
        CallError, Req,
        monitor::{CallDecision, ClientMonitor, RecvDecision, ReqReceiverMonitor},
    },
};
...
async fn forward_to_local_client() {
    ...
    let forwarding = wokio::spawn(async move {
        let mut req_rx = req_rx;
        req_rx.forward(target_client).await
    });
...
async fn forward_to_remote_client() {
    ...
    let forwarding = wokio::spawn(async move {
        let mut req_rx = req_rx;
        req_rx.forward(remote_client).await
    });
