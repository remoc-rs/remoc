...
//!
//! `TraitServerShared` and `TraitServerSharedMut` require the target object to be
//! [`Send`] + [`Sync`] and they are also [`Send`] + [`Sync`] themselves and the
//! futures returned by [`ServerShared::serve`] and [`ServerSharedMut::serve`] are [`Send`].
//! The other servers only require of the target object what the methods of the trait
//! need: [`Send`] when a method takes `self` or `&mut self` and [`Sync`] when a
//! method takes `&self`.
//!
...
/// # Thread safety
///
/// The generated server is [`Send`] when `Target` is [`Send`] and [`Sync`] when
/// `Target` is [`Sync`].
///
/// `Target` must be [`Send`] when the remote trait has a method taking `self` or
/// `&mut self` and [`Sync`] when it has a method taking `&self`.
///
/// # Compatibility
///
/// This trait is meant to be implemented only by the code generated by the
...
/// # Thread safety
///
/// The generated server is both [`Send`] and [`Sync`] when `Target` is [`Sync`],
/// and neither otherwise; `Target` need not be [`Send`].
///
/// This server is only generated for remote traits whose methods all take `&self`,
/// which requires `Target` to be [`Sync`].
///
/// # Compatibility
///
/// This trait is meant to be implemented only by the code generated by the
...
/// `Target` is [`Sync`].
///
/// `Target` must be [`Send`] when the remote trait has a method taking `&mut self`
/// and [`Sync`] when it has a method taking `&self`.
///
/// # Compatibility
///
/// This trait is meant to be implemented only by the code generated by the
...
/// This trait is meant to be implemented only by the code generated by the
/// [remote attribute](remote). Implementing it by hand is not supported.
pub trait ServerShared<Target, Codec>: ServerBase + Send + Sync
where
    Self: Sized,
    Self::Client: Clone,
{
...
/// This trait is meant to be implemented only by the code generated by the
/// [remote attribute](remote). Implementing it by hand is not supported.
pub trait ServerSharedMut<Target, Codec>: ServerBase + Send + Sync
where
    Self: Sized,
{
    /// Creates a server and the client connected to it with the default request buffer size.
...
/// This trait is meant to be implemented only by the code generated by the
/// [remote attribute](remote). Implementing it by hand is not supported.
pub trait ReqReceiver<Codec>: ServerBase + Send + Sync
where
    Self: Sized,
{
